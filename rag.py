...
def big_llm(system, user):
    ...
    r = requests.post(f"{OLLAMA}/api/chat", json={
        ...
        "options": {"num_keep": 0, "temperature": 0, "num_ctx": BIG_NUM_CTX},
...
шаги, требования, значения) — приведи её ПОЛНОСТЬЮ и по пунктам своими словами. НЕ отсылай к
приложению, перечню или другому документу, если сами пункты присутствуют в контексте — собери
ответ из них. Ссылку («см. приложение N…») давай только если самих пунктов в контексте нет.
